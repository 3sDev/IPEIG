                ...
                <div class="card-body">
                  ...
                  <form action="{{ url('attendances') }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de sauvegarder ces données?')">
                    ...
                    <div class="row">
                      ...
                      <input type="text" style="display:none;" name="date_absence" id="date_absence" value="{{ $dateAbs }}"><br>
                      <input type="text" style="display:none;" name="seance_absence" id="seance_absence" value="{{ $seanceAbs }}"><br>
                      <input type="text" style="display:none;" name="classe_id" id="classe_id" value="{{ $classeId }}">
                      ...
                      <div class="col-lg-2">
                          <div class="form-group">
                              ...
                              <select name="teacher_id" id="teacher_id" class="form-control" required style="width: 100% !important" required> 
                      ...
                      <div class="col-lg-4">
                          <div class="form-group">
                              ...
                              <select name="matiere_id" id="matiere_id" class="form-control" required style="width: 100% !important" required>
...
<script>
  ...
  $('#attendance_seance_fin').change(function() {
      var attendance_seance_fin = $(this).val();
      var attendance_seance_debut = $("#attendance_seance_debut").val();
      var teacher_id = $("#teacher_id").val();
      var matiere_id = $("#matiere_id").val();
      var seance_absence = $("#seance_absence").val();
      var date_absence = $("#date_absence").val();
      var classe_id = $("#classe_id").val();

      //console.log(date_absence);
      $("#containerTable").empty();
      $("#btnAction").empty();

      if (attendance_seance_fin) {
  
          $.ajax({
              type: "GET",
              url: "{{ url('getAttendanceTable') }}?classe_id="+classe_id+"&matiere_id="+matiere_id+"&date_absence="+date_absence+"&attendance_seance_debut="+attendance_seance_debut+"&attendance_seance_fin="+attendance_seance_fin,
              success: function(res) {
  
                  if (res) {

                    $("#btnAction").append('<button type="submit" class="btn btn-secondary" onclick="return IsEmpty();"><b>Envoyer</b></button>');

                    res.map(element=>{

                      $("#containerTable").append('<tr>\
                          <td><input type="text" class="inputStyle" name="student_id[]" value="'+element.idStudent+'" readonly></td>\
                          <td>'+element.nomStudent+" "+element.prenomStudent+'</td>'+
                          (element.ABS == null ? '<td align="center"><select id="selec3" name="presence[]"><option value="P"> P </option><option value="A"> A </option></select></td>' : '<td align="center"><select id="selec3" name="presence[]" readonly style="background-color: #ffa895 "><option value="P"> A </option></select></td>') +
                        '</tr>');
                    });
