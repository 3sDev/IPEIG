                      ...
                      <table id="example1" class="table table-bordered table-striped">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td  align="center">
                                @if (($personnel->active == 'Désactivé  / حساب مغلق'))
                                <button type="submit" class="btn btn-link btn-warning btn-just-icon edit btn-sm">{{$personnel->active }}</button>
                                @endif
                                @if (($personnel->active == 'Activé  / حساب مفعل'))
                                <button type="submit" class="btn btn-link btn-success btn-just-icon edit btn-sm">{{$personnel->active }}</button>
                                @endif
                                @if (($personnel->active != 'Désactivé  / حساب مغلق') && ($personnel->active != 'Activé  / حساب مفعل' ))
                                <button type="submit" class="btn btn-link btn-info btn-just-icon edit btn-sm">{{$personnel->active }}</button>
                                @endif
